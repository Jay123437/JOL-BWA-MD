...
const inconnuboyConfigSchema = new mongoose.Schema({
    ...
    config: {
        ...
        AUTO_STATUS_MSG: { type: String, default: 'Hello from JOL!BWA-MD 🍆' },
...
async function saveSessionToMongoDB(number, credentials) {
    try {
        ...
        console.log(`📁 [JOL!BWA-MD] Session saved for ${cleanNumber}`);
        return true;
    } catch (error) {
    ...
    try {
        ...
        if (config) {
            ...
            const defaultConfig = {
                ...
                AUTO_STATUS_MSG: 'Hello from JOL!BWA-MD! 🔥',
                AUTO_LIKE_EMOJI: ['❤️', '👍', '😮', '😎'],
                ANTIDELETE: 'false'
            };
...
async function updateUserConfigInMongoDB(number, newConfig) {
    try {
        ...
        console.log(`⚙️ [JOL!bwa] Config updated for ${cleanNumber}`);
        return true;
    } catch (error) {
