...
function inconnuboyLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [The TechX by SILVER] ${new Date().toISOString()}: ${message}`);
}
    ...
    try {
            ...
            if (connection === 'open') {
                ...
                if (!existingSession) {
                    await conn.sendMessage(userJid, {
                        ...
                        caption: `\n╭────────────────────◇\n│✦ *_JOL!BWA-MD⚡_ — CONNECTED* 🔥\n│✦ Type *${prefix}menu* to see all commands 💫\n│✦ Prefix 『 ${prefix} 』  Mode 〔${mode}〕\n╰────────────────────○\n*© ᴘᴏᴡᴇʀᴇᴅ ʙʏ _Jolibwa bel bwa🏌🏻‍♂️_*`
                    });
                }
            }
            if (connection === 'close') {
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const newsletterJids = ['120363412457932005@newsletter'];
                const newsEmojis = ['❤️', '👍', '😮', '😎', '💀', '💫', '🔥', '👑'];
...
conn.ev.on('group-participants.update', async (update) => {
    await groupEvents(conn, update);
                ...
                const myquoted = {
                    key: { remoteJid: 'status@broadcast', participant: '50940616026@s.whatsapp.net', fromMe: false, id: createSerial(16).toUpperCase() },
                    message: { contactMessage: {
                        displayName: '© Jolibwa Bel bwa ',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:silver\nORG:The TechX;\nTEL;type=CELL;type=VOICE;waid=50940616026:50940616026\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© SILVER TECH' } }
                    }},
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'JOLIBWA MD is running 🔥', activeSessions: activeSockets.size }));
